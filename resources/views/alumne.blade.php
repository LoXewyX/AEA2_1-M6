    ...
    <table class="table table-bordered mb-5 mt-5">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $alumne->centre->nom }}</td>
                <td>{{ $alumne->ensenyament->nom ?? 'No assignat' }}</td>
                <td>
                    ...
                    <form id="delete-project-{{ $alumne->id }}-form" action="{{ route("alumne.destroy", ["alumne" => $alumne]) }}" method="POST" class="d-none">
                        @method("DELETE")
                        @csrf
                    </form>
                </td>
